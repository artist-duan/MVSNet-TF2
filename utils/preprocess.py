...
def bar(percent, name = '', width = 50):
    if percent > 1: 
        percent = 1
    show_str = (name + ': ' + '[%%-%ds]' %width) %(int(percent*width)*'>')
...
def crop_mvs_input(images, cams, args, depth_image=None, max_w=0, max_h=0):
...
def load_cam(file, max_d = 192, interval_scale=1, ):
...
def gen_dtu_resized_path(dtu_data_folder, num_view = 3, mode = 'training'):
        ...
        if mode == 'training':
            ...
            for j in range(0, 7):
                ...
                for p in range(0, int(cluster_list[0])):
                    ...
                    ref_image_path = os.path.join( image_folder, ('rect_%03d_%d_r5000.png' % ((ref_index + 1), j)))
                    ref_cam_path = os.path.join(cam_folder, ('%08d_cam.txt' % ref_index))
                    paths.append(ref_image_path)
                    paths.append(ref_cam_path)
                    # view images
                    for view in range(num_view - 1):
                        view_index = int(cluster_list[22 * p + 2 * view + 3])
                        view_image_path = os.path.join(image_folder, ('rect_%03d_%d_r5000.png' % ((view_index + 1), j)))
...
def gen_dtu_mvs_path(dtu_data_folder, view_num = 3, mode='training'):
    """ generate data paths for dtu dataset """
    sample_list = []
...
def gen_blendedmvs_path(blendedmvs_data_folder, mode='training_mvs'):
    ...
    for data_name in proj_list:
        ...
        for idx in range(0, image_num):
            ...
            if total_view_num < S.view_num - 1:
                continue
            paths = []
...
def gen_eth3d_path(eth3d_data_folder, mode='training'):
    ...
    for data_name in data_names:
        ...
        dict_list = open(dict_file, 'r').read().split()
        ...
        cluster_list = open(cluster_file, 'r').read().split()
        for p in range(0, int(cluster_list[0])):
            paths = []
            # ref image
            ref_index = int(cluster_list[22 * p + 1])
...
if __name__ == "__main__":
    sample = gen_dtu_resized_path('../mvs_training/dtu')
    print(sample)
    sample = gen_dtu_resized_path('../mvs_training/dtu', mode='validation')
    print(sample)