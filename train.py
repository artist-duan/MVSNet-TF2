...
def train():
    ...
    ''' Datasets '''
    train_dataset = MVSDatasets(args.batch_size, args.train_path, "train", args.num_view, args.num_depth, args.interval_scale)
    test_dataset = MVSDatasets(args.batch_size, args.test_path, "val", args.num_view, args.num_depth, args.interval_scale)
    ...
    learning_rate_fn = tf.keras.optimizers.schedules.ExponentialDecay(args.lr, args.stepvalue, args.gamma, name = "lr")
    ...
    for i in range(start_epoch, args.epochs):
        for b, data in enumerate(train_dataset.dataset):
            ...
            with tf.GradientTape() as tape:
                ...
                # loss = mvsnet_loss(mvsnet, depth_est, depth_refine, depth, mask)
                loss1, less_one1, less_three1 = mvsnet_regression_loss(depth_est, depth, intervals) 
                loss2, less_one2, less_three2 = mvsnet_regression_loss(depth_refine, depth, intervals)           
            ...
            if b % args.summary_interval == 0:
                with writer.as_default():
                    tf.summary.scalar("loss", loss.numpy(), step = index )
                    tf.summary.scalar("less1", less_one2.numpy(), step = index )
                    tf.summary.scalar("less3", less_three2.numpy(), step = index )
                    tf.summary.scalar("lr", learning_rate_fn( index ).numpy(), step = index )
                    writer.flush()
        ...
        '''  Val '''
        test_loss, test_less1, test_less3, count = 0, 0, 0, 0
        for b, data in enumerate(test_dataset.dataset):
            ...
            test_less1 += less_one2
            test_less3 += less_three2
            count += 1
