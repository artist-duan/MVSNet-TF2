...
def probability_filter(dense_folder, prob_threshold, depth_cls = "init"):
    image_folder = os.path.join(dense_folder, 'images')
    depth_folder = os.path.join(dense_folder, 'depths_mvsnet')
    ...
    for image_name in image_names:
        image_prefix = os.path.splitext(image_name)[0]
        init_depth_map_path = os.path.join(depth_folder, image_prefix+'_'+ depth_cls +'.pfm')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--depth_cls', type=str, default = 'init')
    parser.add_argument('--fusibile_exe_path', type=str, default = '/data/fusibile/build/fusibile')
    ...
    probability_filter(dense_folder, prob_threshold, args.depth_cls)
    ...
    # Fusion time: 2.781104326248169s
